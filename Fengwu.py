...
class Fengwu(nn.Module):
    ...
    修改为：输入是两个时间步长为[:,220,181,360]，输出为[:,110,181,360]
    
    Args:
        img_size: Image size(Lat, Lon). Default: (181, 360)
        pressure_level: Number of pressure_level. Default: 21
    ...
    def __init__(
        self,
        # params,
        in_shape=(2, 110, 181, 360),  # 修改为两个时间步
        pressure_level=21,
        embed_dim=192,
        patch_size=(8, 8),
        # num_heads=(6, 12, 12, 6),
        ...
        **kwargs
        ...
        self.time_fusion = nn.Sequential(
            nn.Conv2d(220, 110, kernel_size=1, stride=1, padding=0),  # 将两个时间步的通道融合为单个时间步
            nn.GELU(),
            nn.Conv2d(110, 110, kernel_size=3, stride=1, padding=1),
            nn.GELU(),
        )
    ...
    def forward(self, x):
        """
        Args:
            x (torch.Tensor): 输入两个时间步的数据 [B, 220, 181, 360]
            
        Returns:
            output (torch.Tensor): 输出单个时间步的预测 [B, 110, 181, 360]
        """
        # 首先通过时间融合模块将两个时间步的信息融合
        x = self.time_fusion(x)  # [B, 110, 181, 360]
        ...
        reshaped = torch.cat((u, v, r, t, z), dim=1)
...
if __name__ == '__main__':
    # 测试修改后的模型
    inputs = torch.randn(8, 220, 181, 360)  # 两个时间步的输入
    model = Fengwu().to('cuda')
    output = model(inputs.to('cuda'))
    print(f"输入形状: {inputs.shape}")
    print(f"输出形状: {output.shape}")
    print(f"模型参数数量: {sum(p.numel() for p in model.parameters())/1024/1024} G")