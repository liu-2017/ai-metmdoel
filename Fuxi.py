...
class Fuxi(nn.Module):
    ...
    Args:
        ...
        num_groups (Sequence[int] | int, optional): number of groups to separate the channels into.1536
        num_heads (int, optional): Number of attention heads.
        window_size (int | tuple[int], optional): Local window size.
    """
    def __init__(self, img_size=(2, 181, 360), patch_size=(2, 4, 4), in_chans=110, out_chans=110,
                 embed_dim=256, num_groups=32, num_heads=8, window_size=7):
        super().__init__()
...
if __name__ == "__main__":
    ...
    output = fuxi(x)
    print(output.shape)
    torch.save(Fuxi.state_dict(), 'model.pth')