...
class Pangu(Module):
    def __init__(
        ...
        self.patchembed2d = PatchEmbed2D(
            img_size=img_size,
            patch_size=patch_size[1:],  # (8, 8)
            in_chans=4,  # 单时刻地面数据4个通道
            embed_dim=embed_dim,
        )
        
        self.patchembed3d = PatchEmbed3D(
            img_size=(21, img_size[0], img_size[1]),  # (21, 181, 360)
            patch_size=patch_size,  # (2, 8, 8)
            in_chans=5,  # 单时刻高空数据5个通道
            embed_dim=embed_dim,
        )
        
        # 计算patch后的维度
        # 地面数据: 181/8=22.625 -> 向上取整为23, 360/8=45
        # 高空数据: 21/2=10.5 -> 向上取整为11, 181/8=23, 360/8=45
        patch_lat = math.ceil(img_size[0] / patch_size[1])  # 23
        patch_lon = math.ceil(img_size[1] / patch_size[2])  # 45
        
        # 总平面数: 地面1个平面 + 高空11个平面 = 12个平面
        patched_inp_shape = (12, patch_lat, patch_lon)  # (12, 23, 45)
        ...
        patched_inp_shape_downsample = (
            12,
            math.ceil(patched_inp_shape[1] / 2),  # 12
            math.ceil(patched_inp_shape[2] / 2),  # 23
        )
        ...
        self.patchrecovery2d = PatchRecovery2D(
            img_size, patch_size[1:], 2 * embed_dim, 5
        )
        self.patchrecovery3d = PatchRecovery3D(
            (21, img_size[0], img_size[1]), patch_size, 2 * embed_dim, 5
        )
        
        # 添加时间融合层
        self.time_fusion_2d = nn.Conv2d(10, 4, kernel_size=1)  # 两个时刻地面数据融合为单时刻
        self.time_fusion_3d = nn.Conv3d(10, 5, kernel_size=1)  # 两个时刻高空数据融合为单时刻

    def forward(self, x):
        """
        Args:
            x (torch.Tensor): [batch, 218, lat, lon] - 两个时刻的数据
        """
        batch_size = x.shape[0]
        
        # 分离两个时刻的数据
        # 时刻1: 通道 0-108 (109个通道)
        # 时刻2: 通道 109-217 (109个通道)
        time1 = x[:, :110, :, :]
        time2 = x[:, 110:, :, :]
        
        # 处理时刻1
        upper_air_time1 = time1[:, :21*5, :, :].reshape(
            batch_size, 5, 21, time1.shape[2], time1.shape[3]
        )
        surface_time1 = time1[:, 21*5:, :, :]  # [batch, 4, lat, lon]
        
        # 处理时刻2
        upper_air_time2 = time2[:, :21*5, :, :].reshape(
            batch_size, 5, 21, time2.shape[2], time2.shape[3]
        )
        surface_time2 = time2[:, 21*5:, :, :]  # [batch, 4, lat, lon]
        
        # 合并两个时刻的数据
        surface_combined = torch.cat([surface_time1, surface_time2], dim=1)  # [batch, 8, lat, lon]
        upper_air_combined = torch.cat([upper_air_time1, upper_air_time2], dim=1)  # [batch, 10, 21, lat, lon]
        
        # 时间融合
        surface_fused = self.time_fusion_2d(surface_combined)  # [batch, 4, lat, lon]
        upper_air_fused = self.time_fusion_3d(upper_air_combined)  # [batch, 5, 21, lat, lon]
        
        # 通过PatchEmbed层
        surface_embedded = self.patchembed2d(surface_fused)  # [batch, embed_dim, patch_lat, patch_lon]
        upper_air_embedded = self.patchembed3d(upper_air_fused)  # [batch, embed_dim, patch_pl, patch_lat, patch_lon]
        
        # 获取维度信息
        B, C_surface, Lat_surface, Lon_surface = surface_embedded.shape  # [batch, 96, 23, 45]
        B, C_upper, Pl_upper, Lat_upper, Lon_upper = upper_air_embedded.shape  # [batch, 96, 11, 23, 45]
        
        # 检查维度是否匹配
        if Lat_surface != Lat_upper or Lon_surface != Lon_upper:
            raise ValueError(f"维度不匹配: 地面({Lat_surface}, {Lon_surface}) vs 高空({Lat_upper}, {Lon_upper})")
        
        # 扩展地面数据的维度以匹配高空数据
        # 地面数据扩展为 [batch, embed_dim, 1, patch_lat, patch_lon]
        surface_expanded = surface_embedded.unsqueeze(2)  # [batch, embed_dim, 1, 23, 45]
        
        # 合并地面和高空数据
        x_combined = torch.cat([surface_expanded, upper_air_embedded], dim=2)  # [batch, embed_dim, 12, 23, 45]
        
        B, C, Pl_total, Lat, Lon = x_combined.shape
        
        # 验证维度是否正确
        assert Pl_total == 12, f"期望12个平面，实际得到{Pl_total}个平面"
        assert Lat == 23, f"期望Lat=23，实际得到{Lat}"
        assert Lon == 45, f"期望Lon=45，实际得到{Lon}"
        
        # 重塑为Transformer输入格式
        x = x_combined.reshape(B, C, -1).transpose(1, 2)  # [batch, Pl_total*Lat*Lon, C]
        ...
        # 准备输出
        output = torch.cat([x, skip], dim=-1)  # [batch, Pl_total*Lat*Lon, 2*C]
        output = output.transpose(1, 2).reshape(B, -1, Pl_total, Lat, Lon)  # [batch, 2*C, 12, 23, 45]
        
        # 分离地面和高空输出
        output_surface = output[:, :, 0, :, :]  # 地面数据 [batch, 2*C, 23, 45]
        output_upper_air = output[:, :, 1:, :, :]  # 高空数据 [batch, 2*C, 11, 23, 45]
        
        # 恢复原始分辨率
        output_surface = self.patchrecovery2d(output_surface)  # [batch, 4, 181, 360]
        output_upper_air = self.patchrecovery3d(output_upper_air).reshape(batch_size, 5*21, 181, 360)  # [batch, 105, 181, 360]


        # 合并输出
        output_combined = torch.cat([output_upper_air, output_surface], dim=1)  # [batch, 109, 181, 360]
...
if __name__ == '__main__':
    ...
    input = torch.randn(1, 220, 181, 360)
    output = net(input)
